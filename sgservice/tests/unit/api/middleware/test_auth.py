...
import sgservice.api.middleware.auth
from sgservice.tests import base


class TestSGServiceKeystoneContextMiddleware(base.TestCase):

    def setUp(self):
        super(TestSGServiceKeystoneContextMiddleware, self).setUp()

        @webob.dec.wsgify()
        def fake_app(req):
            self.context = req.environ['sgservice.context']
            return webob.Response()

        self.context = None
        self.middleware = (sgservice.api.middleware.auth
                           .SGServiceKeystoneContext(fake_app))

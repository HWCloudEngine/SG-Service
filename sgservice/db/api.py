...
def replication_update(context, id, values):
    ...
    Raises ReplicationNotFound if backup with the given ID doesn't exist.
    """
...
def volume_get(context, id, columns_to_join=[]):
    ...
    Raises VolumeNotFound if volume with the given ID doesn't exist.
    """
    return IMPL.volume_get(context, id, columns_to_join)

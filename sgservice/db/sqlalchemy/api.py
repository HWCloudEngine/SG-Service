...
def volume_destroy(context, volume_id):
    ...
    with session.begin():
        volume_ref = _volume_get(context, volume_id, session=session)
        volume_ref.delete(session)


@require_context
def volume_get(context, volume_id, columns_to_join=[]):
...
def replication_destroy(context, replication_id):
    ...
    with session.begin():
        replication = model_query(context, models.Replication). \
            filter_by(id=replication_id)
        if not replication:
            raise exception.ReplicationNotFound(replication_id=replication_id)
        else:
            replication.update(updated_values)
...
def _reps_get_query(context, session=None, project_only=False):
    return model_query(context, models.Replication, session=session,
                       project_only=project_only)


def _process_reps_filters(query, filters):
    if filters:
...
def snapshot_destroy(context, snapshot_id):
    ...
    updated_values = {'status': fields.SnapshotStatus.DELETED,
                      ...
                      'updated_at': literal_column('updated_at')}
    with session.begin():
        snapshot = model_query(context, models.Snapshot). \
            filter_by(id=snapshot_id)
        if not snapshot:
            raise exception.SnapshotNotFound(snapshot_id=snapshot_id)
        else:
            snapshot.update(updated_values)
...
PAGINATION_HELPERS = {
    models.Volume: (
        _volume_get_query, _process_volume_filters, _volume_get),
    models.Snapshot: (
        _snaps_get_query, _process_snaps_filters, _snapshot_get),
    models.Backup: (
        _backups_get_query, _process_backups_filters, _backup_get),
    models.Replication: (
        _reps_get_query, _process_reps_filters, _replication_get)
}

...
"""sgservice common internal object model"""
...
class SGServiceObjectRegistry(base.VersionedObjectRegistry):
...
class SGServiceObject(base.VersionedObject):
    OBJ_SERIAL_NAMESPACE = 'sgservice_object'
    OBJ_PROJECT_NAMESPACE = 'sgservice'

    def sgservice_obj_get_changes(self):
    ...
    def __contains__(self, name):
        ...
        # "'foo' in obj" tests.
        return name in self.obj_extra_fields or super(SGServiceObject,
                                                      self).__contains__(name)


class SGServiceObjectDictCompat(base.VersionedObjectDictCompat):
    ...
    def get(self, key, value=base._NotSpecifiedSentinel):
        ...
        if key not in self.obj_fields:
            ...
            # rely on it.
            LOG.debug('SGService object %(object_name)s has no '
...
class SGServicePersistentObject(object):
...
class SGServiceComparableObject(base.ComparableVersionedObject):
    def __eq__(self, obj):
...
class SGServiceObjectSerializer(base.VersionedObjectSerializer):
    OBJ_BASE_CLASS = SGServiceObject

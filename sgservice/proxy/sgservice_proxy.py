...
class SGServiceProxy(manager.Manager):
    ...
    def init_host(self, **kwargs):
        ...
        for volume in volumes:
            if volume.status == fields.VolumeStatus.ENABLING \
                    or volume.status == fields.VolumeStatus.DISABLING:
        ...
        for volume_id, volume in self.sync_volumes.items():
            ...
            try:
                ...
                if type(exc) == NotFound:
                    ...
                    volume.destroy()
                    self.volumes_mapping.pop(volume_id)
    ...
    def _get_csd_volume_id(self, volume_id):
        ...
        if volume_id in self.volumes_mapping.keys():
            return self.volumes_mapping[volume_id]
    ...
    def enable_sg(self, context, volume_id):
        ...
        # step 3: add to map cache
        self.volumes_mapping[volume_id] = csd_volume_id

        # step 4: add to sync status list

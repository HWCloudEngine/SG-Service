...
class ClientFactory(object):
    ...
    @classmethod
    def create_client(cls, service, context, **kwargs):
        if not cls._factory:
            cls._factory = {}
            for module in cls._list_clients():
                service_name = module.split('.')[-1]
                module = importutils.import_module(module)
                cls._factory[service_name] = module

        return cls._factory[service].create(context, **kwargs)

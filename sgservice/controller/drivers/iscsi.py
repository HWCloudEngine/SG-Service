...
class ISCSISGDriver(SGDriver):
    ...
    def restore_backup(self, backup, size, device):
        response = self.sg_ctrl.backups.restore_backup(backup, size, device)
        if response['status'] != 0:
    ...
    def create_volume(self, snapshot, device):
        response = self.sg_ctrl.snapshots.create_volume(snapshot, device)
        if response['status'] != 0:
            msg = _("Call create_volume to sg client failed")
            LOG.error(msg)
            raise exception.SGDriverError(reason=msg)

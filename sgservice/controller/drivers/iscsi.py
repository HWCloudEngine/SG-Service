...
class ISCSIDriver(SGDriver):
    ...
    def replicate_ctrl(self):
        return ReplicateCtrl(self._host, self._port)
    ...
    def delete_snapshot(self, snapshot):
        ...
        try:
            res = self.snap_ctrl.DeleteSnapshot(snap_type, vol_id, snap_id,
                                                checkpoint_uuid)
        except Exception as exc:
    ...
    def rollback_snapshot(self, snapshot):
        ...
        try:
            res = self.snap_ctrl.RollbackSnapshot(snap_type, vol_id, snap_id,
                                                  checkpoint_uuid)
        except Exception as exc:
